...
import { CatalogService, CompensationService, IndicatorService } from '@app/core/services/models';
import { UtilityService, IBaseReportParams } from '@app/core';

@Component({
  selector: 'app-trend-plain',
  templateUrl: './trend-plain.component.html',
  styleUrls: ['./trend-plain.component.scss']
})
export class TrendPlainComponent
  ...
  ngOnInit() {
    ...
    this._indicatorService.getVariablesDictionary().subscribe(
      data => this.indicatorList = data.map(item => {
        const r = { key: item.value, value: item.value };
        return r;
      })
    );
  ...
  mockData(value: IBaseReportParams) {
    this.logs = this._us.getMockDataReport(value.startDate, value.endDate, this.userIdentification || value.identification, value.schema, value.indicator);
  }
}
